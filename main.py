...
def reset_timer():
    ...
    reps = 0
    title_label.config(text="Timer", font=(FONT_NAME, 40,))
...
def start_timer():
    ...
    if reps % 2 == 1:
        count_down(work_sec)
        title_label.config(text="WORK", fg=GREEN, font=(FONT_NAME, 30))
        ...
        if reps % 8 == 0:
            ...
            title_label.config(text="SHORT BREAK", fg=PINK, font=(FONT_NAME, 30))
# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
...
check_mark_label.config(bg=YELLOW, fg=GREEN,)
